...
class DrillingHydraulicsEngine:
    ...
    @staticmethod
    def calculate_cuttings_slip_velocity(mud_weight_ppg: float, plastic_viscosity_cp: float,
                                         cuttings_diameter_in: float = 0.25,
                                         cuttings_density_ppg: float = 21.0) -> float:
        """
        Stokes slip velocity (ft/min) for spherical cuttings.
        """
        # Convert to SI
        rho_f = mud_weight_ppg * 1000 / 8.34   # kg/m³
        rho_p = cuttings_density_ppg * 1000 / 8.34
        d = cuttings_diameter_in * 0.0254      # m
        mu = plastic_viscosity_cp * 0.001      # Pa·s
        g = 9.81
        # Stokes law
        v_slip = (d**2 * (rho_p - rho_f) * g) / (18 * mu)  # m/s
        return v_slip * 196.85  # convert to ft/min
    ...
    def calculate_bit_hydraulics(self, mud_weight_ppg: float) -> Dict[str, float]:
        if not self.nozzles:
            return {"tna_sq_in": 0.0, "bit_pressure_drop_psi": 0.0, "jet_velocity_fps": 0.0,
                    "hydraulic_horsepower_hhp": 0.0, "jif_lbf": 0.0}
