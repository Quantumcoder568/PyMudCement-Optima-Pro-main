...
Implements Bingham Plastic, Power Law and Herschel-Bulkley models with
laminar/turbulent regime detection and proper TVD-based ECD.
"""
import math
import logging
...
class DrillingHydraulicsEngine:
    def __init__(
        ...
        Parameters
        ----------
        total_depth_ft : Measured Depth (MD) of the well.
        true_vertical_depth_ft : True Vertical Depth (TVD). If None, assumes
            a vertical well (TVD = MD). ECD and hydrostatic calculations
            always use TVD as required by industry standards.
        """
        self.surface_mud_weight_ppg = max(0.1, surface_mud_weight_ppg)
        self.flow_rate_gpm = max(0.1, flow_rate_gpm)
        self.total_depth_ft = max(1.0, total_depth_ft)          # MD
        # TVD – critical for correct ECD (syllabus equation uses z = TVD)
    ...
    # ------------------------------------------------------------------
    # Flow-regime detection (Reynolds number)
    # ------------------------------------------------------------------
    @staticmethod
    def _reynolds_annular(
        ...
        """
        Approximate Reynolds number for Bingham plastic in an annulus.
        Re = 928 * MW * V * dh / (PV + YP*dh/(V*5.0))   (field units)
        """
    ...
    # ------------------------------------------------------------------
    # Friction gradients – laminar & turbulent
    # ------------------------------------------------------------------
    def calculate_annular_friction_gradient(
        self, seg: WellSegment, v_ann_fpm: float
    ):
        """
        Returns (dp/dl in psi/ft, flow_regime_string).
        Uses laminar handbook formulas when Re ≤ 2100 and a turbulent
    ...
    def calculate_pipe_friction_gradient(
        self, seg: WellSegment, v_pipe_fpm: float
    ):
    ...
    def calculate_cuttings_slip_velocity(
        ...
        """
        Moore-type slip velocity (ft/min) – more realistic than pure Stokes
        for typical drilling cuttings.
        """
        ...
        g = 9.81
        v_stokes = (d ** 2 * (rho_p - rho_f) * g) / (18.0 * mu)  # m/s
        v_slip_ms = v_stokes * 0.7  # conservative engineering factor
        return max(v_slip_ms * 196.85, 1.0)  # ft/min
    ...
    def solve(self) -> Dict[str, Any]:
        ...
        # ★ Critical fix: ECD always uses True Vertical Depth
...
class DiagnosticEngine:
    ...
    def analyze_telemetry(
        self, physics_metrics: Dict[str, Any], historical_esd: float
    ) -> Dict[str, Any]:
