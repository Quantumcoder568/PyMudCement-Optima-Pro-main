    ...
    if st.button("Run Engineering Calculations", type="primary", use_container_width=True):
        with st.spinner("Solving hydraulics..."):
            try:
                ...
                st.markdown('<div class="section-title" style="margin-top:1.4rem;"><i class="fas fa-chart-simple"></i> Key Hydraulics Metrics</div>', unsafe_allow_html=True)
                ...
                st.markdown('<div class="section-title" style="margin-top:1.6rem;"><i class="fas fa-list-ul"></i> Segment Analytics</div>', unsafe_allow_html=True)
                ...
                if "gradient_df" in st.session_state and not st.session_state.gradient_df.empty:
                    ...
                    if not gdf.empty:
                        try:
                            ...
                            st.markdown('<div class="section-title" style="margin-top:1.6rem;"><i class="fas fa-shield-halved"></i> Formation Pressure Integrity</div>', unsafe_allow_html=True)
                            ...
                            if ecd > safe["fracture"]:
                                st.error(f"**CRITICAL** · ECD {ecd:.2f} ppg exceeds fracture gradient {safe['fracture']:.2f} ppg.")
                                ...
                                st.warning(f"ECD {ecd:.2f} ppg approaching fracture limit {safe['fracture']:.2f} ppg.")
                            elif ecd < safe["pore"]:
                                st.warning(f"ECD {ecd:.2f} ppg below pore pressure {safe['pore']:.2f} ppg – influx risk.")
                            else:
                                st.success(f"ECD {ecd:.2f} ppg within safe window [{safe['min_mw_ppg']:.2f} – {safe['max_mw_ppg']:.2f}] ppg.")
                        except Exception as e:
                ...
                if ratio < 1.5:
                    st.warning(f"Low cuttings transport ratio ({ratio:.2f}). Consider increasing flow rate.")
                else:
...
with tab2:
    ...
    fig.update_layout(
        scene=dict(
            ...
            zaxis=dict(backgroundcolor=scene_bg, gridcolor=grid_c, color=axis_c, zerolinecolor=grid_c, autorange="reversed"),
        ),
...
with tab4:
    ...
    if st.button("Run Cementing Design", key="cement_btn", type="primary", use_container_width=True):
        with st.spinner("Calculating cement job..."):
            try:
                ...
                st.markdown('<div class="section-title" style="margin-top:1.4rem;"><i class="fas fa-flask"></i> Cementing Job Volumes</div>', unsafe_allow_html=True)
                ...
                st.markdown('<div class="section-title" style="margin-top:1.4rem;"><i class="fas fa-flask"></i> Suggested Additives</div>', unsafe_allow_html=True)
    ...
    if st.button("Compare with Industry Benchmarks", key="bench_btn"):
        if st.session_state.cementing_results and st.session_state.cementing_params:
            ...
            comp = compare_cementing_results(result, p["casing_od"], p["hole_dia"], p["interval_ft"])
            st.markdown('<div class="section-title" style="margin-top:1.4rem;"><i class="fas fa-chart-bar"></i> Industry Benchmark Comparison</div>', unsafe_allow_html=True)
            if "error" in comp:
