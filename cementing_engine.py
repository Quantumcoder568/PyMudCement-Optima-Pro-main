...
All volumes are calculated dynamically from geometry (no hard-coded spacer).
"""
import math
...
class PrimaryCementingInput(BaseModel):
    ...
    washout_factor_pct: float = Field(
        default=15.0, ge=0, description="Open-hole excess factor % (e.g., 15 for 15%)"
    )
    ...
    # Spacer design – either length of annular coverage or explicit volume
    spacer_annular_length_ft: float = Field(
        default=500.0,
        ge=0,
        description="Annular length of spacer coverage (ft). Volume is calculated from geometry.",
    )
    spacer_volume_override_bbl: Optional[float] = Field(
        default=None,
        ge=0,
        description="If set, overrides the calculated spacer volume (bbl).",
    )

    # Optional TVD for hydrostatic / plug-bump calculations
    true_vertical_depth_ft: Optional[float] = Field(
        default=None,
        ge=0,
        description="True Vertical Depth (ft). If None, interval_length_ft is used as TVD.",
    )
...
class CementingEngine:
    ...
    def calculate_annular_volume_bbl(
        ...
        casing_od_in: float,
        length_ft: float,
        washout_pct: float = 0.0,
    ) -> float:
        """
        Annular volume (bbl) – matches the syllabus equation:
        V_ann = (π/4) * (D_hole² − d_casing²) * L * (1 + W_e)   [converted to bbl]
        """
        ...
        vol_cu_ft = (
            (math.pi / 4.0)
            * ((hole_dia_in ** 2 - casing_od_in ** 2) / 144.0)
            * length_ft
            * (1.0 + we)
        )
        return vol_cu_ft / 5.6146  # ft³ → bbl

    @staticmethod
    def calculate_pipe_capacity_bbl(pipe_id_in: float, length_ft: float) -> float:
        """Internal volume capacity of a pipe/casing string in barrels."""
    ...
    def compare_with_industry(
        ...
        """Wrapper around benchmarks.compare_cementing_results."""
        from benchmarks import compare_cementing_results

        return compare_cementing_results(
            software_results, casing_od, hole_dia, interval_length
        )

    def design_primary_job(self, params: PrimaryCementingInput) -> Dict[str, Any]:
        """
        Calculates volumes for lead/tail slurry, spacer, displacement,
        and recommended plug-bumping pressure.
        """
        lead_length = max(0.0, params.interval_length_ft - params.tail_slurry_length_ft)

        # --- Annular slurry volumes ---
        tail_vol_bbl = self.calculate_annular_volume_bbl(
            params.hole_diameter_in,
            params.casing_od_in,
            params.tail_slurry_length_ft,
            params.washout_factor_pct,
        )
        lead_vol_bbl = (
        ...
        shoe_track_vol_bbl = self.calculate_pipe_capacity_bbl(
            params.casing_id_in, params.shoe_track_length_ft
        )
        displacement_vol_bbl = self.calculate_pipe_capacity_bbl(
            params.casing_id_in,
            max(0.0, params.interval_length_ft - params.shoe_track_length_ft),
        )

        total_tail_slurry_bbl = tail_vol_bbl + shoe_track_vol_bbl

        # --- Spacer volume (dynamic – no hard-code) ---
        if params.spacer_volume_override_bbl is not None:
            ...
            spacer_vol_bbl = self.calculate_annular_volume_bbl(
                params.hole_diameter_in,
                params.casing_od_in,
                params.spacer_annular_length_ft,
                washout_pct=0.0,
            )
            spacer_method = "calculated_from_annular_length"

        # --- Plug bumping pressure (uses TVD when supplied) ---
        tvd = (
            params.true_vertical_depth_ft
            if params.true_vertical_depth_ft is not None
            else params.interval_length_ft
        )
        diff_hydrostatic_psi = (
            (params.tail_slurry_density_ppg - params.displacement_fluid_density_ppg)
            * 0.052
            * tvd
        )
    ...
    def design_abandonment_plug(
        ...
        """Volume and hydrostatic parameters for open-hole P&A / side-track plugs."""
        plug_vol_cu_ft = (math.pi / 4.0) * ((hole_dia_in ** 2) / 144.0) * plug_length_ft
        plug_vol_bbl = plug_vol_cu_ft / 5.6146
        sacks_of_cement = plug_vol_cu_ft / 1.18  # API Class G typical yield

        hydrostatic_gain_psi = (
            (slurry_density_ppg - mud_density_ppg) * 0.052 * plug_length_ft
        )
